class Hxtools < Formula
  ...
  def install
    on_macos do
      inreplace "doc/hxtools.7", "control autonomous CDDA playback using Linux ioctls", "disabled on this system"
  ...
  def caveats
    <<~EOS
      The 'hcdplay' binary is not installed on macOS on as it is not supported by the operating system
    EOS
  end
